...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    @Query("SELECT COUNT(m) > 0 FROM Message m WHERE " +
           "(m.sender.username = :u1 AND m.receiver.username = :u2) OR " +
           "(m.sender.username = :u2 AND m.receiver.username = :u1)")
    boolean existsConversationBetween(@Param("u1") String u1, @Param("u2") String u2);
    ...
    @Query("SELECT m FROM Message m WHERE " +
           "((m.sender.username = :u1 AND m.receiver.username = :u2) OR " +
            "(m.sender.username = :u2 AND m.receiver.username = :u1)) " +
            "AND m.attachmentType IS NOT NULL AND m.attachmentType <> 'audio' " +
            "AND m.attachmentFilename IS NOT NULL " +
            "ORDER BY m.timestamp DESC")
    List<Message> findSharedMedia(@Param("u1") String u1, @Param("u2") String u2, Pageable pageable);
