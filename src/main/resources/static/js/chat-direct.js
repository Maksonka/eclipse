...
function updateTotalUnreadBadge() {
    ...
    if (total > 0) {
        if (totalEl) {
            ...
            const title = document.querySelector('.sidebar-conversations-wrap .sidebar-section-title');
            if (title) {
...
stompClient.connect({}, function () {
    ...
    stompClient.subscribe('/user/queue/chat-error', function (payload) {
        handleChatError(JSON.parse(payload.body));
    });
...
function getVoiceAudioEl(messageId) {
    if (!messagesContainer) return null;
    var row = messagesContainer.querySelector('[data-message-id="' + messageId + '"]');
    if (!row) return null;
    return row.querySelector('.voice-player audio') || row.querySelector('audio[data-voice-player]');
}
